...
public class DedicatedFile extends AbstractFile {
	...
	public AbstractFile createFile(APDU apdu) {
		...
		case FILE_TYPE_TRANSPARENT:
			if (fid == 0 || fid == 0x100){
				f = new CHVFile(fid, this, tmpAC, fileSize);
			}else
			f = new ElementaryFile(fid, this, tmpAC, fileSize);
			break;
		case FILE_TYPE_VAR_RECORD:
			f = new VariableLinearFile(fid, this, tmpAC, numberOfRecords);
			break;
		case FILE_TYPE_CYCLIC:
			byte recordSize = apdu.getBuffer()[RECORD_SIZE_OFFSET];
			f = new CyclicFile(fid, this, tmpAC, numberOfRecords, recordSize);
			break;
		case FILE_TYPE_DF:
			f = new DedicatedFile(fid, this, tmpAC);
			break;
		default:
			ISOException.throwIt(ISO7816.SW_DATA_INVALID);
		}
		f.setActive(activated);
